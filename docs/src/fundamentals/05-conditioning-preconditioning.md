...
ϕ_inc = 0.0
E0 = 1.0
polarization = :θ       # vertical polarization
...
The complete script is available as `examples/ex_conditioned_solve.jl` in the repository. You can run it directly to reproduce the results:

```bash
julia examples/ex_conditioned_solve.jl
```
...
- **`examples/ex_conditioned_solve.jl`** (to be created) – a complete worked example of assembling an EFIE matrix, diagnosing conditioning, applying regularization/preconditioning, and verifying the results (as described in Section 6).
- **`examples/ex_low_frequency_stabilization.jl`** (if present) – illustrates conditioning for low‑frequency EFIE problems.
...
- **Coding 1**: See `examples/ex_conditioned_solve.jl` for a template.
